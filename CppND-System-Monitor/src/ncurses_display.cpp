...
void NCursesDisplay::DisplaySystem(System& system, WINDOW* window) {
  ...
  if (num_proc > 1){
    for (unsigned int i=0; i < num_proc - 1; i++){
      mvwprintw(window, ++row, 2, "CPU: %d", i);
      wattron(window, COLOR_PAIR(1));
      mvwprintw(window, row, 10, "");
      wprintw(window, ProgressBar(system.Cpu().Utilization(i+1)).c_str());
    }
  }
...
void NCursesDisplay::DisplayProcesses(std::vector<Process>& processes,
  ...
  int const proc_column{60};
  int const threads_column{74};
  int const command_column{90};
  ...
  mvwprintw(window, row, proc_column, "PROCESSOR");
  mvwprintw(window, row, threads_column, "NR THREADS");
  ...
  for (int i = 0; i < n; ++i) {
    ...
    mvwprintw(window, row, proc_column,
